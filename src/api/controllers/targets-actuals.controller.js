    ...
    getProduct(pRequest, pResponse) {
        try {
            ...
            if (vSelectedTab == "Day") {
                ...
                ormS.query("SELECT mpc.brand, mpc.category_id,mpc.category_name, mpsc.sub_category_id, mpsc.sub_category_name, ceil(cast(mpt.target_qty as float)/" + vDaysInMonth + ") AS target_qty, COALESCE(SUM(transLoad.amount),0)+COALESCE(SUM(transProd.quantity),0) AS amountActual, ceil(((COALESCE(SUM(transLoad.amount),0)+COALESCE(SUM(transProd.quantity),0)) / (COALESCE(ceil(NULLIF(mpt.target_qty,0)))))*100) as prc_round, round((((COALESCE(SUM(transLoad.amount),0)+COALESCE(SUM(transProd.quantity),0)) / (COALESCE(ceil(NULLIF(mpt.target_qty,0)))))*100)::numeric ,2)::float8 as prc_round FROM mst_prod_cat AS mpc LEFT OUTER JOIN mst_prod_sub_cat mpsc ON mpc.category_id = mpsc.category_id LEFT OUTER JOIN mst_product AS mp ON mp.sub_category_id = mpsc.sub_category_id LEFT OUTER JOIN (SELECT ts.order_date, tsload.product_id,tsload.amount FROM trx_saleord_load_det AS tsload JOIN trx_sales_order ts ON ts.order_id = tsload.order_id WHERE ts.order_date = '" + vtoday + "' AND ts.dsp_id = " + vUserId + ") AS transLoad ON transload.product_id = mp.product_id LEFT OUTER JOIN ( SELECT ts.order_date, tsprod.product_id,tsprod.quantity FROM trx_saleord_prd_det AS tsprod JOIN trx_sales_order ts ON ts.order_id = tsprod.order_id WHERE ts.order_date = '" + vtoday + "'  AND ts.dsp_id = " + vUserId + ")AS transProd ON transProd.product_id = mp.product_id LEFT OUTER JOIN mst_target AS mpt ON mp.product_id = mpt.product_id WHERE mpt.dsp_id = '" + vUserId + "' AND mpt.target_month = '" + vmonth + "' AND mpt.target_year = '" + vyear + "' GROUP BY mpc.category_id,mpsc.sub_category_id , mpt.target_qty ORDER BY mpc.category_id, mpsc.sub_category_id ASC;", { type: ormS.QueryTypes.SELECT })
                ...
                ormS.query("SELECT mpc.brand, mpc.category_id,mpc.category_name, mpsc.sub_category_id, mpsc.sub_category_name, ceil(cast(mpt.target_qty as float)/" + vWeeksInMonth + ") AS target_qty ,COALESCE(SUM(transLoad.amount),0)+COALESCE(SUM(transProd.quantity),0) AS amountActual, ceil(((COALESCE(SUM(transLoad.amount),0)+COALESCE(SUM(transProd.quantity),0)) / (COALESCE(ceil(NULLIF(mpt.target_qty,0)))))*100) as prc_round, round((((COALESCE(SUM(transLoad.amount),0)+COALESCE(SUM(transProd.quantity),0)) / (COALESCE(ceil(NULLIF(mpt.target_qty,0)))))*100)::numeric ,2)::float8 as prc_round FROM mst_prod_cat AS mpc LEFT OUTER JOIN mst_prod_sub_cat mpsc ON mpc.category_id = mpsc.category_id LEFT OUTER JOIN mst_product AS mp ON mp.sub_category_id = mpsc.sub_category_id LEFT OUTER JOIN (SELECT ts.order_date, tsload.product_id,tsload.amount FROM trx_saleord_load_det AS tsload JOIN trx_sales_order ts ON ts.order_id = tsload.order_id WHERE extract(month from ts.order_date) = extract(month from current_date) AND  extract(day from ts.order_date) BETWEEN '" + first + "' AND '" + last + "' AND  extract(year from ts.order_date) = extract(year from current_date) AND ts.dsp_id = " + vUserId + ") AS transLoad ON transload.product_id = mp.product_id LEFT OUTER JOIN ( SELECT ts.order_date, tsprod.product_id,tsprod.quantity FROM trx_saleord_prd_det AS tsprod JOIN trx_sales_order ts ON ts.order_id = tsprod.order_id WHERE extract(month from ts.order_date) = extract(month from current_date) AND  extract(day from ts.order_date) BETWEEN '" + first + "' AND '" + last + "' AND  extract(year from ts.order_date) = extract(year from current_date) AND ts.dsp_id = " + vUserId + ")AS transProd ON transProd.product_id = mp.product_id LEFT OUTER JOIN mst_target AS mpt ON mp.product_id = mpt.product_id WHERE mpt.dsp_id = '" + vUserId + "' AND mpt.target_month = '" + vmonth + "' AND mpt.target_year = '" + vyear + "' GROUP BY mpc.category_id,mpsc.sub_category_id , mpt.target_qty ORDER BY mpc.category_id, mpsc.sub_category_id ASC;", { type: ormS.QueryTypes.SELECT })
                    .then(function (pProdCats) {
                    var vResult = {
                        "status": "Success",
                        "statusMessage": "",
